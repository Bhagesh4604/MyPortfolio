...
export const NeuralNoise: React.FC<NeuralNoiseProps> = ({
  ...
  const FRAG = `
    ...
    void main() {
      ...
      // AI-native palette: Indigos, Limes, and Slates
      vec3 base = normalize(vec3(
        0.1,
        0.7 * abs(cos(2.0 * u_scroll_progress)),
        0.8 * abs(sin(2.0 * u_scroll_progress))
      ));
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      scrollProgress.current = window.pageYOffset / (document.body.scrollHeight - window.innerHeight || 1);
    };
